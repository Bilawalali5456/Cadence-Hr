...
export function LeavePage({
  ...
  const isExecTl = isTl && isExecutiveRole(currentUser.role);
  const myTeamIds = new Set(
    (users || []).filter(u => u.teamLeadId === currentUser.id).map(u => u.id)
  );
  ...
  // Team Lead (non-Executive) main list = own requests only; team shown separately.
  const visibleReqs = isTl && !isExecutiveRole(currentUser.role)
    ? allVisible.filter(r => r.userId === currentUser.id)
    : allVisible;
  // Employee TL: API scopes team data. Executive TL: filter by assigned members.
  const teamLeaveReqs = isTl
    ? (requests || []).filter(r => {
        if (!r?.userId || r.userId === currentUser.id) return false;
        if (isExecTl) return myTeamIds.has(r.userId);
        return true;
      })
    : [];
  const teamShortReqs = isTl
    ? (shortLeaveRequests || []).filter(r => {
        if (!r?.userId || r.userId === currentUser.id) return false;
        if (isExecTl) return myTeamIds.has(r.userId);
        return true;
      })
    ...
    <div className="space-y-5 max-w-3xl">
      ...
      {isTl && (
        <Card className="overflow-hidden">
          <div className="px-5 py-3 border-b border-slate-200">
            <h3 className="text-sm font-semibold" style={{ color: B.dark }}>
              {isExecTl ? "My Team" : "Team Leaves"}
            </h3>
            <p className="text-xs text-slate-400 mt-0.5">
              {isExecTl
                ? "Leave and short-leave requests from your assigned team members."
                : "Read-only — only Executives approve leave requests."}
            </p>
          </div>
          {teamLeaveReqs.length === 0 && teamShortReqs.length === 0 ? (
            ...
            <div className="divide-y divide-slate-100">
              {teamLeaveReqs.map(r => (
                <div
                  key={r.id}
                  className={`px-5 py-3 flex items-center gap-3 flex-wrap ${isExecTl ? "bg-sky-50/60" : ""}`}
                >
                  <Avatar name={r.empName} />
                  <div className="flex-1 min-w-40">
                    <div className="text-sm font-medium text-slate-800">{r.empName}</div>
